...
<body>
  ...
  <div class="page_wrapper">
    <div style="background-image: url(system_image/background1.jpg); background-position-y: 60%;" class="first_banner_1">
      <div class="image_container_1">
        <div class="text_area">
          <h1 class="text-heading" style="">Your Ads</h1>
        </div>
      </div>
    </div>
          ...
          <div style="align-self: center" class="card border-0 bg-light mt-2 d-flex p-1">
            <ol class="breadcrumb pt-2">
              ...
              <li class="breadcrumb-item active" aria-current="page">
                <a style="font-size: 16px" href="update_ads.php">Update Ads</a>
              </li>
            </ol>
          </div>
        </nav>
        <div class="row my-4">
          <div class="col-md-4">
            <div class="filter_box px-lg-4">
              ...
              <form autocomplete="off" method="POST" action="search_result.php">
                <div class="form-group search_1">
                  <div class="d-flex justify-content-between align-item-center">
                    ...
                    <button type="button" class="btn btn-warning btn-sm"> <a style="color: inherit;" href="update_ads">Refresh
                        Normal</a></button>
                  </div>
                  <div style="
                          height: 2px;
              ...
              <form action="update_ads.php" method="POST">
                <div class="form-group search_1">
                ...
                <div class="form-group filter_1">
                  ...
                  <div class="range_box d-flex mt-1">
                    <input style="width: 100%" class="custom-range" type="range" step="1000" min="0" name="range"
                      max="<?php echo $price_new?>" oninput="this.nextElementSibling.value = this.value"
                      id="customRange1" />
                    <output class="text-secondary" style="font-weight: 500; font-size: 18px">
                      <?php echo $price_new?>
                    </output>
              ...
              <div class="btn_sorting d-flex mb-2">
                <?php
                  ...
                  if(isset($_SESSION['email'])){
                    $user_ad = $_SESSION['email'];
                    
                    if(isset($_POST['location'])){
                      require_once('components/connect.php');
                      $location = $_POST['locationing'];
                      $loc_sql = "SELECT * FROM product_ads WHERE location = '$location' AND user_email = '$user_ad'";
                    ...
                    elseif(isset($_POST['ranging'])){
                      $range = $_POST['range'];
                      $loc_sql = "SELECT * FROM product_ads WHERE price <= '$range' AND user_email = '$user_ad'";
                      $result = mysqli_query($conn, $loc_sql);
                      ...
                      $sql = "SELECT * FROM product_ads WHERE user_email = '$user_ad'";
                        ...
                        <div class="filter_box1 px-1">
                          ...
                          <form autocomplete="off" method="POST" action="search_result.php">
                            <div class="form-group search_1">
                              <div class="d-flex justify-content-between align-item-center">
                                <h4 class="text-secondary">Search</h4>
                                <button type="button" class="btn btn-warning btn-sm"> <a href="update_ads.php"> Refresh</a></button>
                              </div>
                        ...
                        "></div>
                              <div class="range_box d-flex mt-1">
                                <input style="width: 100%" class="custom-range" type="range" step="1000" min="0"
                                  name="range" max="<?php echo $price_new?>"
                                  oninput="this.nextElementSibling.value = this.value" id="customRange1" />
                                <output class="text-secondary" style="font-weight: 500; font-size: 18px">
                                  <?php echo $price_new?>
                                </output>
                                <p class="text-secondary" style="font-weight: 500">
                ...
                <?php
                  if(isset($_SESSION['email'])){
                    ...
                    if(isset($_POST['location'])){
                      require_once('components/connect.php');
                      $location = $_POST['locationing'];
                      $sql = "SELECT * FROM product_ads WHERE location = '$location' AND user_email = '$user_ad'";
                      $result = mysqli_query($conn, $sql);
                          ...
                          while($row = mysqli_fetch_assoc($result)){
                            if($row['title'] !== ''){
                              if($row['premium_nature'] == 1 ){
                                ...
                                updateCard($row['images'], $row['title'], $row['price'], $date_exact, $row['location'], $row['id']);
                    ...
                    elseif(isset($_POST['ranging'])){
                      $range = $_POST['range'];
                      $rng_sql = "SELECT * FROM product_ads WHERE price <= '$range' AND user_name = '$user_ad'";
                      ...
                      $user_ad = $_SESSION['username'];

                     
                      $sql = "SELECT * FROM product_ads WHERE user_name = '$user_ad'";
                      $result = mysqli_query($conn, $sql);
