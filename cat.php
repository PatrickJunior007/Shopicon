          ...
          <div style="align-self: center" class="card border-0 bg-light mt-2 d-flex p-1">
            <ol class="breadcrumb pt-2">
              ...
              <li class="breadcrumb-item active" aria-current="page">
                <a style="font-size: 16px" href="cat.php">Category</a>
              </li>
            </ol>
          </div>
        </nav>
              ...
              <form autocomplete="off" method="POST" action="search_result.php">
                <div class="form-group search_1">
                  <div class="d-flex justify-content-between align-item-center">
                    ...
                    <button type="button" class="btn btn-warning btn-sm"> <a style="color: inherit;" href="cat.php">Refresh Normal</a></button>
                  </div>
                  <div style="
                          height: 2px;
                          width: 50px;
          ...
          <div class="col-md-8">
            <div class="row">
                        ...
                        <div class="filter_box1 px-1">
                          ...
                          <form autocomplete="off" method="POST" action="search_result.php">
                            <div class="form-group search_1">
                              <div class="d-flex justify-content-between align-item-center">
                                <h4 class="text-secondary">Search</h4>
                                <button type="button" class="btn btn-warning btn-sm"> <a href="cat.php"> Refresh</a></button>
                              </div>
              ...
              <div class="row px-0 mx-0 grid">
                <?php
                  ...
                  if(isset($_POST['location'])){
                    require_once('components/connect.php');
                    $location = $_POST['locationing'];
                    $sql = "SELECT * FROM product_ads WHERE location = '$location' LIMIT $start_from,$num_per_page";
                    $result = mysqli_query($conn, $sql);
                    $booom = mysqli_num_rows($result);
                    if ($result){
                  ...
                  elseif(isset($_POST['ranging'])){
                    $range = $_POST['range'];
                    $rng_sql = "SELECT * FROM product_ads WHERE price <= '$range' LIMIT $start_from,$num_per_page";
                    ...
                    $sql = "SELECT * FROM product_ads LIMIT $start_from,$num_per_page";
                    $result = mysqli_query($conn, $sql);
                    $booom = mysqli_num_rows($result);
              ...
              <?php
                ...
                if(isset($_POST['location'])){
                  $pr_query ="SELECT * FROM product_ads WHERE location = '$location'";
                  //Execution
                  $pr_result = mysqli_query($conn,$pr_query);
                  $pi = mysqli_num_rows($pr_result);
                  
                  $result1 = $conn->query("SELECT count(id) AS id FROM product_ads WHERE location = '$location'");
                  ...
                  $pr_query = "SELECT * FROM product_ads WHERE price <= '$range'";
                     //Execution
                  $pr_result = mysqli_query($conn,$pr_query);
                  $pi = mysqli_num_rows($pr_result);
                  
                  $result1 = $conn->query("SELECT count(id) AS id FROM product_ads WHERE price <= '$range'");
                  $custCount = $result1->fetch_all(MYSQLI_ASSOC);
                  $total = $custCount[0]['id'];
                  ...
                  $pr_query = "SELECT * FROM product_ads";
                  //Execution
                  $pr_result = mysqli_query($conn,$pr_query);
                  $pi = mysqli_num_rows($pr_result);
                  
                  $result1 = $conn->query("SELECT count(id) AS id FROM product_ads");
                  $custCount = $result1->fetch_all(MYSQLI_ASSOC);
                  $total = $custCount[0]['id'];
                ...
                <nav  id="containing " aria-label="...">
                  <ul class="pagination table-dark">
                    <li class="page-item <?php if($page == 1){ echo "disabled";}else{ echo "";}?>">
                      <a class="page-link" href="cat.php?page=<?= $Previous; ?>" aria-label="Previous">
                        <span aria-hidden="true">&laquo; Previous</span>
                      </a>
                    </li>
                    <?php for($i = 1; $i<= $total_page; $i++) : ?>
				    	        <li class="page-item <?php if(($booom < $start_from)||($booom > 2)){ echo "";}else{ echo "disabled";}?>" aria-current="page"><a class="page-link " href="cat.php?page=<?= $i; ?>"><?= $i; ?></a></li>
				            <?php endfor; ?>

                    <li class="page-item <?php if($booom > 2){ echo "";}else{ echo "disabled";}?> ">
                      <a class="page-link" href="cat.php?page=<?= $Next; ?>" aria-label="Next">
